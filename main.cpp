	   ...
	   if(up.at<uchar>(t2.pos) == 255) {
		   ...
		   for(int i=-1; i<=1; i+=2) {
			for(int j=-1; j<=1; j+=2) {
				...
				// Existiert der Punkt überhaupt? Pixel am Rand haben ja nur 3 bzw. an den Ecken 2 4-connected Nachbarn
		   ...
		   // TODO: über region iterieren, Durchschnittsfarbe berechnen, und jeden Pixel mit dieser Farbe einfärben!
...
int main( int argc, char** argv )
{
  ...
  // Über alle Pixel iterieren, um sie in die PriorityQueue einzusortieren
  for(int y=0;y<grad.rows;y++)
	{
		for(int x=0;x<grad.cols;x++)
		{
			// Gradient für jeden Pixel auslesen
			int gradient = grad.at<uchar>(Point(x,y));

			// usedPixels wird als weißes Bild initialisiert, hier wird später gespeichert, welche Pixel beim Auslesen der Priority Queue schon verwendet wurden
